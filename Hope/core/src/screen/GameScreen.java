...
public class GameScreen implements Screen {
	 ...
	 */
	public GameScreen(Hope hp) {

		this.hp = hp;
		this.level = new Level(hp);
	...
	public void render(float delta) {
		...
		switch (level.getHero().getDeplacementsInt()) {
		case 1: // Up
			heroUpRegion = level.getHero().getHeroUp()
					.getKeyFrame(level.getHero().getStateTime(), true);
			batch.draw(heroUpRegion, level.getHero().getPositionX(), level
					.getHero().getPositionY());
			break;
		case 2: // down

			heroDownRegion = level.getHero().getHeroDown()
					.getKeyFrame(level.getHero().getStateTime(), true);
			batch.draw(heroDownRegion, level.getHero().getPositionX(), level
					.getHero().getPositionY());

			break;
		case 3:// right

			heroRightRegion = level.getHero().getHeroRight()
					.getKeyFrame(level.getHero().getStateTime(), true);
			batch.draw(heroRightRegion, level.getHero().getPositionX(), level
					.getHero().getPositionY());

			break;
		case 4:// left
			heroLeftRegion = level.getHero().getHeroLeft()
					.getKeyFrame(level.getHero().getStateTime(), true);
			batch.draw(heroLeftRegion, level.getHero().getPositionX(), level
					.getHero().getPositionY());
	...
	public void show() {
		...
		/********************* MUSIQUE *******************************/
		sound = Gdx.audio.newMusic(Gdx.files.internal("music.wav"));
